...
RSpec.describe PhasesController, type: :controller do
  ...
  describe 'GET #index' do
    ...
    let(:params) { { user_id: user.id, lead_id: lead.id } }
  ...
  describe 'GET #show' do
    ...
    let!(:params) { { user_id: user.id, lead_id: lead.id, id: phase.id } }
  ...
  describe 'GET #edit' do
    context 'with authorized user (business_developer)' do
      let!(:user) { create(:user) }
      let!(:lead) { create(:lead, :with_phases, user: user) }
      let!(:params) { { user_id: user.id, lead_id: lead.id, id: lead.phases.first.id } }
  ...
  describe 'POST #create' do
    context 'with authorized user' do
      ...
      let!(:lead) { create(:lead, :with_phases, user: user) }
      let(:params) do
        { lead_id: lead.id, phase: { phase_type: 'Test', assignee: user_manager.email,
      ...
      it 'will not create phase as assignee is not technical_manager' do
        params[:phase][:assignee] = user.email
        expect do
          post :create, params: params
  ...
  describe 'PUT/PATCH #update' do
    context 'with authorized user (Owner of phase , Business Developer)' do
      ...
      let!(:lead) { create(:lead, :with_phases, user: user) }
      let(:params) do
        { user_id: user.id, lead_id: lead.id, id: lead.phases.first.id, phase: { phase_type: 'Interview' } }
      end
  ...
  describe 'POST #destroy' do
    ...
    context 'with unauthorized user (not owner of the phase)' do
      ...
      let!(:user_bd) { create(:user) }
      let!(:lead) { create(:lead, :with_phases, user: user_bd) }
      let(:params) { { lead_id: lead.id, id: lead.phases.first.id } }
  ...
  describe 'POST #engineer' do
    context 'when signed in as technical manager' do
      ...
      let(:params) do
        { user_id: user.id, lead_id: lead.id, id: lead.phases.first.id, engineer: { user_id: user_eng.id } }
      end
    ...
    context 'when engineer is already assigned' do
      let!(:user) { create(:user, email: lead.phases.first.assignee) }
      let!(:user_bd) { create(:user) }
      let!(:lead) { create(:lead, :with_phases, user: user_bd) }
      let(:user_eng) { create(:user, :engineer) }
      let(:params) do
        { user_id: user.id, lead_id: lead.id, id: lead.phases.first.id,
  ...
  describe 'GET#complete' do
    context 'when phase is not complete' do
      let!(:user) { create(:user) }
      let!(:lead) { create(:lead, :with_phases, user: user) }
      let(:params) { { user_id: user.id, lead_id: lead.id, id: lead.phases.first.id, phase: lead.phases.first } }
